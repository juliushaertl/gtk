                 ...
                 GdkRectangle *area)
{
  ...
  gtk_widget_get_allocation (GTK_WIDGET (paned), &widget_allocation);
  gtk_widget_get_allocation (priv->handle_widget, area);

  if (!gtk_paned_get_wide_handle (paned))
    extra = HANDLE_EXTRA_SIZE;

  area->x -= widget_allocation.x + extra;
  area->y -= widget_allocation.y + extra;
  area->width += extra * 2;
  area->height += extra * 2;
}
      ...
      if (priv->orientation == GTK_ORIENTATION_HORIZONTAL)
        {
          ...
          child2_allocation.width = MAX (1, allocation->width - priv->child1_size - priv->handle_pos.width);
